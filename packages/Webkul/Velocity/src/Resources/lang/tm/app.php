<?php

return [
    'admin'         => [
        'system'    => [
            'velocity' => [
                'general'  => 'Общее',
                'category'  => 'Категория',
                'settings'  => 'Настройки',
                'extension_name' => 'Тема Велосити',
                'error-module-inactive' => 'Предупреждение: Статус темы велосити неактивный',
            ],

            'settings' => [
                'channels'=> [
                    'subscription_bar' => 'Содержание панели подписки'
                ],
            ],

            'general' => [
                'status' => 'Статус',
                'active' => 'Активный',
                'inactive' => 'Неактивный',
            ],
            'category' => [
                'all' => 'Все',
                'left' => 'Влево',
                'right' => 'Вправо',
                'active' => 'Активный',
                'custom' => 'Custom',
                'inactive' => 'Неактивный',
                'image-alignment' => 'Выравнивание изображения',
                'icon-status' => 'Статус иконки категории',
                'image-status' => 'Статус изображения категории',
                'sub-category-show' => 'Показать подкатегории',
                'image-height' => 'Высоста изображения [в пикселях]',
                'image-width' => 'Ширина изображения [в пикселях]',
                'show-tooltip' => 'Показать всплывающую подсказку категории',
                'num-sub-category' => 'Количество подкатегорий',
            ]
        ],
        'layouts'   => [
            'velocity' => 'Велосити',
            'cms-pages' => 'Страницы CMS',
            'meta-data' => 'Мета данные',
            'category-menu' => 'Меню категорий',
            'header-content' => 'Контент заголовка',
        ],
        'contents'  => [
            'self' => 'Свой',
            'active' => 'Активировать',
            'new-tab' => 'Новая вкладка',
            'inactive' => 'Неактивный',
            'title' => 'Список контента',
            'select' => '-- Выбрать --',
            'add-title' => 'Добавить контент',
            'btn-add-content' => 'Добваить контент',
            'save-btn-title' => 'Сохранить контент',
            'autocomplete' => '[Autocomplete]',
            'no-result-found' => 'Записей не найдено.',
            'search-hint' => 'Искать товары здесь...',
            'mass-delete-success' => 'Выбранный контент удален успешно.',
            'tab' => [
                'page' => 'Настройка страниц',
                'content' => 'Настройки контента',
                'meta_content' => 'Мета данные',
            ],
            'page' => [
                'title' => 'Заголовок',
                'status' => 'Статус',
                'position' => 'Позиция',
            ],
            'content' => [
                'content-type' => 'Тип контента',
                'custom-title' => 'Заголовок кастома',
                'category-slug' => 'Slug категории',
                'link-target' => 'Целевая ссылка на страницу',
                'custom-product' => 'Товары маркета',
                'custom-heading' => 'Название кастома',
                'catalog-type' => 'Тип каталога продуктов',
                'static-description' => 'Описание контента',
                'page-link' => 'Ссылка страницы [e.g. http://example.com/../../]',
            ],
            'datagrid' => [
                'id' => 'Id',
                'title' => 'Заголовок',
                'status' => 'Статус',
                'position' => 'Позиция',
                'content-type' => 'Тип контента',
            ]
        ],
        'meta-data' => [
            'footer'                    => 'Футер',
            'title'                     => 'Мета данные Велосити',
            'activate-slider'           => 'Активировать слайдер',
            'home-page-content'         => 'Контент Главной страницы',
            'footer-left-content'       => 'Контент слева на футере',
            'subscription-content'      => 'Контент панели подписок',
            'sidebar-categories'        => 'Сайдбар категорий',
            'header_content_count'      => 'Счетчик контента хедера',
            'footer-left-raw-content'   => '<p>ИП «Парахат» - одно из крупнейших современных высокотехнологичных предприятий по выпуску натуральных соков и нектаров, холодного чая, газированных напитков, фруктово-овощных пюре, кисломолочных продуктов, концентратов, мясных изделий, работающее по самым передовым технологиям. Предприятие было основано в 1993 году. .</p>',
            'slider-path'               => 'Путь слайдера',
            'category-logo'             => 'Лого категории',
            'product-policy'            => 'Политика товара',
            'update-meta-data'          => 'Обновить мета данные',
            'product-view-image'        => 'Изображение продукта',
            'advertisement-two'         => 'Реклама два изображения',
            'advertisement-one'         => 'Реклама одно изображение',
            'footer-middle-content'     => 'Контент в центре футера',
            'advertisement-four'        => 'Реклама четыре изображения',
            'advertisement-three'       => 'Реклама три изображения',
            'images'                    => 'Изображения',
            'general'                   => 'Общее',
            'add-image-btn-title'       => 'Добавить изображение',
            'footer-middle' => [
                'about-us'  => 'О нас',
                'customer-service' => 'Обслуживание клиентов',
                'whats-new' => 'Что нового',
                'contact-us' => 'Связаться с нами',
                'order-and-returns' => 'Заказы и возвраты',
                'payment-policy' => 'Условия оплаты',
                'shipping-policy' => 'Условия доставки',
                'privacy-and-cookies-policy' => 'Политика конфиденциальности'
            ]
        ],
        'category'  => [
            'save-btn-title' => 'Меню сохранить',
            'title' => 'Список меню категорий',
            'add-title' => 'Добавить контент меню',
            'edit-title' => 'Редактировать контент меню ',
            'btn-add-category' => 'Добавить контент категории',
            'datagrid' => [
                'category-id' => 'Id категории',
                'category-name' => 'Название категории',
                'category-icon' => 'Иконка категории',
                'category-status' => 'Статус',
            ],
            'tab' => [
                'general' => 'Общее',
            ],
            'status' => 'Статус',
            'active' => 'Активный',
            'inactive' => 'Неактивный',
            'select' => '-- Выбрать --',
            'icon-class' => 'Класс иконки',
            'select-category' => 'Выбрать категорию',
            'tooltip-content' => 'Контент всплывающей подсказки',
            'mass-delete-success' => 'Меню выбранных категорий успешно удалено.',
        ],
        'general'   => [
            'locale_logo' => 'Логотип локальной настройки',
        ],
    ],

    'home' => [
        'view-all'           => 'Ählisini seretmek',
        'add-to-cart'        => 'Sebede goşmak',
        'hot-categories'     => 'Gyzgyn bölümler',
        'payment-methods'    => 'töleg usuly',
        'customer-reviews'   => 'Müşderleriň synlary',
        'shipping-methods'   => 'eltip bermegiň görnüşleri',
        'popular-categories' => 'Ýörgünli категории',
    ],

    'header'        => [
        'cart'              => 'Sebet',
        'guest'             => 'Myhman',
        'logout'            => 'Ulgamdan çykmak',
        'title'             => 'Hasap',
        'account'           => 'hasap',
        'profile'           => 'Profil',
        'wishlist'          => 'Halanylanlar',
        'all-categories'    => 'Ähli bölümler',
        'search-text'       => 'Harytlary şu ýerde gözläň',
        'welcome-message'   => ' :customer_name hoş geldiňiz!',
        'dropdown-text'     => 'Halanylanlary, sargytlary we sebedi dolandyrmak',
    ],

    'menu-navbar'   => [
        'text-more'     => 'Has köp',
        'text-category' => 'Bölümlere görä söwdalar',
    ],

    'minicart'      => [
        'cart'      => 'Sebet',
        'view-cart' => 'Sebede seretmek',
    ],

    'checkout'      => [
        'qty'       => 'Mukdary',
        'checkout'  => 'Sargydy amala aşymak',
        'cart'      => [
            'view-cart'     => 'Sebede seretmek',
            'cart-summary'  => 'Sebet barada gysgaça maglumat',
        ],
        'items'     => 'Harytlar',
        'subtotal'  => 'jemi',
        'sub-total' => 'jemi ',
        'proceed'   => 'Amala aşyrmaga geçmek',
    ],

    'customer'      => [
        'compare'           => [
            'text'                  => 'Deňeşdirmek',
            'compare_similar_items' => 'Meňzeş harytlary deňeşdirmek',
            'add-tooltip'           => 'Deňeşdiriş sanawyna harytlary goşmak',
            'added'                 => 'Haryt deňeşdirme sanawyna üstünlikli goşuldy',
            'already_added'         => 'Haryt eýýäm deňeşdirme sanawynda bar',
            'removed'               => 'Haryt deňeşdirme sanawynda üstünlikli aýryldy',
            'removed-all'           => 'Ähli harytlar deňeşdirme sanawyndan üstünlikli aýryldy',
            'empty-text'            => "Deňeşdiriş sanawyňyzda önüm ýok.",
            'product_image'         => 'Harydyň suraty',
            'actions'               => 'Hereketler',
        ],
        'login-form'        => [
            'sign-up'               => 'Ulgama agzama bolmak',
            'new-customer'          => 'Täze müşderi',
            'customer-login'        => 'Müşderiniň ulanyjy ady',
            'registered-user'       => 'hasaba alnan ulanyjylar',
            'your-email-address'    => 'Siziň elektron poçtaňyz',
            'form-login-text'       => 'Eger siziň hasabyňyz bar bolsa, öz ulanyjy elektron poçtaňyz bilen ulgama giriň',
        ],
        'signup-form'       => [
            'login'             => 'Ulanyjy ady',
            'become-user'       => 'Ulanyjy bolmak',
            'user-registration' => 'Ulanyjyny hasaba almak',
            'form-sginup-text'  => 'Bu biziň bazarymyzda ilkinji gezek bolsa, sizi alyjy hökmünde görýänimize begenýäris.',
        ],
        'forget-password'   => [
            'login'                 => '',
            'forgot-password'       => 'Açar sözüni unutdyňyzmy?',
            'recover-password'      => 'Açar sözüni dikeltmek',
            'recover-password-text' => 'Eger açar sözüni unudan bolsaňyz elektron poçtaňyzy görkezip, ony dikeldip bilersiňiz',
        ],
        'wishlist' => [
            'remove-all-success' => 'Islgler sanawynda ähli harytlar üstünlikli aýryldy',
        ],
    ],

    'error'         => [
        'go-to-home'            => 'Baş sahypa dolanmak',
        'page-lost-short'       => 'Mazmuny ýitiriler sahypa',
        'something_went_wrong'  => 'Bir ýalňyşlyk ýüze çykdy',
        'page-lost-description' => "Gözleýän sahypaňyz elýeterli däl.. Ýene gözläp görüň ýa-da Yza düwmesini ulanyň",
],

'products'      => [
    'text'              => 'Harytlar',
    'details'           => 'Jikme-jiklikler',
    'reviews-title'     => 'Synlar',
    'reviewed'          => 'Tassyklanan',
    'review-by'         => 'tassyklanan',
    'quick-view'        => 'Ýüzleý seretmek',
    'not-available'     => 'Elýeter däl',
    'submit-review'     => 'Syn goşmäk',
    'ratings'           => ':totalRatings BAllar',
    'reviews-count'     => ':totalReviews Synlar',
    'customer-rating'   => 'Satyn alyjylar sanawy',
    'more-infomation'   => 'giňişleýin maglumat',
    'view-all-reviews'  => 'Ähli synlara seretmek',
    'write-your-review' => 'ÖZ synyňy ýazmak',
    'short-description' => 'Gysga beýannama',
    'recently-viewed'   => 'Ýakynda görlen harytlar',
    'be-first-review'   => 'Ilkinji syn ýazyjy, siz boluň',
    'new'               => 'täze',
    'sale'=>'aksiýa'
],

'shop'          => [
    'gender'    => [
        'male'   => 'Erkek',
        'other'  => 'Başga',
        'female' => 'Aýal',
    ],
    'general'   => [
        'no'                     => 'Ýok',
        'yes'                    => 'Hawa',
        'view'                   => 'Seretmek',
        'filter'                 => 'Tertiplemek',
        'orders'                 => 'Sargytlar',
        'update'                 => 'Täzelemek',
        'reviews'                => 'Synlar',
        'download'               => 'Ýükläp almak',
        'currencies'             => 'Pul birligi',
        'addresses'              => 'Salgylar',
        'top-brands'             => 'Iň gowy brendler',
        'new-password'           => 'Täze açar sözi',
        'no-file-available'      => 'Elýeter faýl ýok',
        'downloadables'          => 'Göçürip alyp bolýan faýllar',
        'confirm-new-password'   => 'Täze açar sözi tassyklaň',
        'enter-current-password' => 'Häzirki açar sözini giriziň',

        'alert' => [
            'info'      => 'Maglumat',
            'error'     => 'Ýalňyşlyk',
            'success'   => 'Üstünlikli',
            'warning'   => 'Bildiriş',
        ],
    ],
    'wishlist'  => [
        'add-wishlist-text'     => 'Harydy halanlarym sanawyna goşmak',
        'remove-wishlist-text'  => 'Halanlarym sanawynda harydy aýyrmak'
    ]
],

'responsive'    => [
    'header' => [
        'done'      => 'Ýerine ýetirildi',
        'languages' => 'Diller',
        'greeting'  => 'Hoş geldiňiz :customer  !',
    ]
],
]

?>
