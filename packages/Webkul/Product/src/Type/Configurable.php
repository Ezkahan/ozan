...
class Configurable extends AbstractType
{
     ...
     * @var array
     */
    protected $additionalViews = ['admin::catalog.products.accordians.images', 'admin::catalog.products.accordians.categories', 'admin::catalog.products.accordians.variations', 'admin::catalog.products.accordians.channels', 'admin::catalog.products.accordians.product-links', 'admin::catalog.products.accordians.videos'];
    ...
    public function getMinimalPrice($qty = null)
    {
        ...
        $result = ProductFlat::join('products', 'product_flat.product_id', '=', 'products.id')
            ...
            ->selectRaw(
                "IF( {$tablePrefix}product_flat.special_price_from IS NOT NULL
    ...
    {
        if ($this->haveOffer()) {
            return '<div class="sticker sale">' . trans('shop::app.products.sale') . '</div>' . '<span class="price-label">' . trans('shop::app.products.price-label') . '</span>' . '<span class="regular-price">' . core()->currency($this->getMinimalPrice()) . '</span>' . '<span class="final-price">' . core()->currency($this->getOfferPrice()) . '</span>';
        } else {
            return '<span class="price-label">' . trans('shop::app.products.price-label') . '</span>' . ' ' . '<span class="final-price">' . core()->currency($this->getMinimalPrice()) . '</span>';
        }
    }
    ...
    public function getAdditionalOptions($data)
    {
        ...
        foreach ($this->product->super_attributes as $attribute) {
            $option = $attribute
                ->options()
                ->where('id', $childProduct->{$attribute->code})
                ->first();
    ...
    public function totalQuantity()
    {
        ...
        $channelInventorySourceIds = core()
            ->getCurrentChannel()
            ->inventory_sources()
            ->where('status', 1)
            ->pluck('id');
